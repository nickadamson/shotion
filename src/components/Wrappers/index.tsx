  ...
  useEffect(() => {
    if (!temporaryValue && object) {
      if (object?.type === "text" || object?.type?.includes("heading")) {
        setTemporaryValue(
          (object as FormattedBlockWRelations)?.details?.richText?.[0]
            ?.plainText
        );
      } else if (
  ...
  return (
    <>
      {object && (
        ...
        >
          {(
            object as FormattedPageWRelations | FormattedBlockWRelations
          )?.children?.map((child) => (
            <Renderer
              key={child.id}
              id={child.id}
...
const UseDatabaseWrapper: FC<{
  ...
  const {
    ...
    isLoading,
    isError,
    updateDatabase: updateMethod,
  } = useDatabase({ databaseId: id });
...
const cleanupAndUpdateObject = async ({
  oldObject,
  key,
  newValue,
  updateMethod,
