import { Dispatch, forwardRef, ReactNode, SetStateAction, useEffect } from "react";
...
const RowDropdownContentWrapper = forwardRef((props: DropdownProps, ref) => {
    const { translateX, translateY, setTransform, setShowDropdown, children } = props;
    useEffect(() => {
        ...
        if (rect.bottom > window.innerHeight || window.innerHeight - rect.bottom < 5) {
            ...
            if (rect.right > window.innerWidth) {
                setTransform(`translateY(${translateY || "-95%"}) translateX(${translateX || "-20%"})`);
            }
        } else {
            // Check if dropdown is past the width of the window size
            if (rect.right > window.innerWidth) {
